...
def analyze_resume(uploaded_file, job_description):  # Changed file to uploaded_file, removed api_key parameter
    try:
        ...
        if uploaded_file.type == "application/pdf":
            ...
            raise ValueError(
                f"Unsupported file type: {uploaded_file.type} for file: {uploaded_file.name}"
            )
        ...
        **Output Format:**
        ...
        if gemini_response_text:
            ...
            try:
                ...
                if start_index != -1 and end_index != -1 and start_index < end_index:
                    tuple_string = gemini_response_text[
                        start_index: end_index + 1
                    ]  # Extract substring including parentheses
                else:
                ...
                overall_fit_percentage_list = parsed_response[
                    2
                ]  # It's a list containing a float
                ...
                param_names = [
                    ...
                    "Skills Match",
                    "Projects Relevance",
                    "Working Experience Relevance",
                    "Educational History Relevance",
                ]
                for i, param_name in enumerate(param_names):
                    ratings[param_names[i]] = {
                        "rating": parameter_ratings_data[i][
                            0
                        ],  # Rating is the first element
                        "justification": parameter_ratings_data[i][
                            1
                        ],  # Justification is the second
                    }

                # Handle Overall Percentage (it's in a list, extract the float):
                if (
                    overall_fit_percentage_list
                    and isinstance(overall_fit_percentage_list, list)
                    and len(overall_fit_percentage_list) > 0
                ):
                    ratings["overall_fit_percentage"] = float(
                        overall_fit_percentage_list[0]
                    )  # Extract float from list
                else:
                    ratings["overall_fit_percentage"] = (
                        None
                    )  # Handle case if percentage is not properly returned
        ...
        if 'temp_image_path' in locals() and os.path.exists(temp_image_path): # Check if temp_image_path was created and exists
            os.remove(temp_image_path) # Delete the temporary image file


def pdf_To_Img(pdf_bytes):  # Changed parameter to pdf_bytes
    doc = pymupdf.open(stream=pdf_bytes, filetype="pdf")  # Open PDF from bytes
    if len(doc) > 1:
        new_im = Image.new("RGB", (4000, 10000))
        i, pix = 0, 0
        while i < len(doc):
            im = doc[i].get_pixmap(
                matrix=pymupdf.Identity, dpi=250
            ).pil_image()
